...
for($curRegion=0; $curRegion < $totalRegionCount;$curRegion++){
    ...
    while(!$filledRegion){
        if($movingRight){
            $sql="SELECT DP0010001,INTPTLAT10 FROM censusPopulationWithBreakout where (statechar = '".$strState."') and (INTPTLON10 between ".strval($currentLng+(($colSpan-1)*$lngPerCol))." and ".strval($currentLng+($colSpan*$lngPerCol)).") and (INTPTLAT10 between ".$swlat." and ".strval($currentLat).") order by INTPTLAT10 desc;";
        }else{
            $sql="SELECT DP0010001,INTPTLAT10 FROM censusPopulationWithBreakout where (statechar = '".$strState."') and (INTPTLON10 between ".strval($currentLng+(($colSpan-1)*$lngPerCol))." and ".strval($currentLng+($colSpan*$lngPerCol)).") and (INTPTLAT10 between ".strval($currentLat)." and ".$nelat.") order by INTPTLAT10 asc;";
        }
        //$handler->debug('sql', $sql);
        $result = mysqli_query($con,$sql);
